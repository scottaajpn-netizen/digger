...
export function Digger({ initial }: { initial: DigResponse | null }) {
  ...
  return <div className="app-shell">
    ...
    </section><footer><span className="brand">digger.</span><p>La curiosité n’a pas de fin de piste.</p><span>{rated} avis · Mémoire locale active</span>{rated > 0 && <div className="reset-actions">{resetPending ? <><button onClick={() => { setFeedback({}); setSaved({}); setResetPending(false); void fetch("/api/memory/feedback", { method: "DELETE" }).catch(() => undefined); setNotice("Tes avis et la mémoire locale ont été réinitialisés."); }}>Confirmer la remise à zéro</button><button onClick={() => setResetPending(false)}>Annuler</button></> : <button onClick={() => setResetPending(true)}>Réinitialiser mes avis</button>}</div>}</footer></main>
  </div>;
}
