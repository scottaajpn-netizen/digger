...
import { fuzzyQuery, mergeSuggestions, normalize, parseStructuredSearch, resolveSeedSuggestion, typoQueries, type SearchResult, type Suggestion } from "./matching";
...
export const providers={
  ...
  async lastfm(q:string,signal:AbortSignal):Promise<Suggestion[]> {
    ...
    const terms=structured
      ? [structured.title,`${structured.title} ${structured.artist}`,q,...typoQueries(structured.title)]
      : [q,words.slice(0,n).join(" "),words.slice(-n).join(" "),...typoQueries(q)];
    const pool:Suggestion[]=[];
    for(const [index,term] of [...new Set(terms)].slice(0,5).entries()) {
      const mergedBefore=mergeSuggestions(q,pool);
      if(index>=3 && (structured
        ? Boolean(resolveSeedSuggestion(structured.title,structured.artist,mergedBefore))
        : (mergedBefore[0]?.matchScore||0)>.85))break;
      const data=await lastFmJson<{results?:{trackmatches?:{track?:SearchTrack[]}}}>(
        "track.search",
        {track:term,limit:structured&&index===0?"100":index>=3?"100":"30"},
        signal,
      );
      ...
      const merged=mergeSuggestions(q,pool);
      if(structured
        ? Boolean(resolveSeedSuggestion(structured.title,structured.artist,merged))
        : (merged[0]?.matchScore||0)>.94)break;
    }
    return pool;
  },
  async discogs(q:string,signal:AbortSignal):Promise<Suggestion[]> {
    ...
    const pool:Suggestion[]=[];
    for(const hit of result?.results?.slice(0,structured?4:2)||[]) {
...
export async function suggest(q:string,signal:AbortSignal,deps=providers):Promise<SearchResult> {
  ...
  for(const [name,provider] of [["MusicBrainz",deps.mb],["Last.fm",deps.lastfm],["Discogs",deps.discogs]] as const) {
    ...
    const resolved=structured
      ? resolveSeedSuggestion(structured.title,structured.artist,best)
      : undefined;
    if(structured
      ? Boolean(resolved)
      : ((best[0]?.matchScore||0)>.94||best.filter(t=>t.matchScore>.85).length>=5))break;
  }
