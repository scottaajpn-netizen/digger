...
const root = {
  id: 10, title: "Compilation", artists: [a(194, "Various")], labels: [label], genres: ["Electronic"], styles: ["UK Garage"], country: "UK", year: 2001, formats: [{ descriptions: ["Compilation"] }], tracklist: [
...
test("Diversity limits shared labels across all Discogs origins, including co-labels", async () => {
  ...
  const ranked = Array.from({ length: 16 }, (_, i) => ({
    ...
    discogs: {
      ...d.discogs,
      trackArtists: [a(3000 + i, `Artist ${i}`)],
      labels: [
        { id: 999, name: "Shared" },
        { id: 2000 + i, name: `Unique ${i}` },
      ],
    },
  }));
...
test("Discogs HTTP authenticates only on fixed API host, caches and spaces requests", async () => {
  ...
  const get = createDiscogsClient({
    token: () => "test-secret", now: () => clock, sleep: async ms => { waits.push(ms); clock += ms; }, fetch: async (url, init) => {
...
test("Discogs HTTP retries transient errors within budget and never caches failures", async () => {
  ...
  const get = createDiscogsClient({
    token: () => "test", now: () => clock, sleep: async ms => { waits.push(ms); clock += ms; }, fetch: async () => {
...
test("Discogs HTTP cancels queued requests without bypassing the shared limiter", async () => {
  ...
  let calls = 0;
  const get = createDiscogsClient({
    token: () => "test", sleep: async () => { }, fetch: async () => {
